...
func TestCheckRegisterWorker(t *testing.T) {
  ...
  workerInit("worker.api.test", Info{Nats:NatsInfo{ReturnArray: true, SubjectStat:"stats", SubjectMetric: "metrics"}})
  workerRun("worker.api.test")
  defer workerClose("worker.api.test")
  
  resP := workersPublicInfo()
